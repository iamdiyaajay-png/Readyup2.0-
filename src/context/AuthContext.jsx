...
export const AuthProvider = ({ children }) => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      if (firebaseUser) {
        ...
        const fallbackUser = {
          ...
          role: (firebaseUser.email === 'admin@readyup.com' || firebaseUser.email?.includes('admin')) ? 'admin' : 'pending',
          status: 'pending',
          profileCompletion: 0,
        ...
        unsubscribeSnapshot = onSnapshot(
          ...
          (docSnap) => {
            if (docSnap.exists()) {
              ...
              merged.isEmailVerified = firebaseUser.emailVerified;
              setUser(merged);
  ...
  const signUpWithEmail = async (email, password, intendedRole = 'student') => {
    setLoading(true);
    if (intendedRole && intendedRole !== 'returning') {
      sessionStorage.setItem('intendedRole', intendedRole);
    }
  ...
  const loginWithEmail = async (email, password, intendedRole = 'student') => {
    setLoading(true);
    if (intendedRole && intendedRole !== 'returning') {
      sessionStorage.setItem('intendedRole', intendedRole);
    }
  ...
  const resetPassword = async (email) => {
    try {
      // Import sendPasswordResetEmail from firebase/auth at the top, we already have it imported? No, let me check. Actually I will just use sendPasswordResetEmail(auth, email). Wait, let's verify if sendPasswordResetEmail is imported. I will just import it.
