...
If a field cannot be determined from the text, use null. skillsMentioned defaults to [].`;
  ...
  try {
    ...
    if (msg.includes('400') || msg.includes('401') || msg.includes('403')) {
      throw new Error(`❌ API Key Error: Invalid Gemini API key. Get one at aistudio.google.com.`, { cause: err });
    }
    if (msg.includes('429')) throw new Error('⚠️ Quota exceeded — wait a minute and retry.', { cause: err });
    throw new Error(`Gemini parse error: ${msg}`, { cause: err });
  }
}
...
If a field cannot be determined, use null.`;
  ...
  try {
    ...
    const msg = err?.message || 'Unknown error';
    if (msg.includes('400') || msg.includes('401') || msg.includes('403') || msg.includes('API_KEY')) {
      throw new Error(`❌ API Key Error (${msg.match(/\d{3}/)?.[0] || 'Unknown'}): Invalid or unauthorized Gemini API key. Please verify your key at aistudio.google.com.`, { cause: err });
    }
    if (msg.includes('404') || msg.includes('not found')) {
      throw new Error(`❌ Model not found. The API key may not have access to ${MODEL}. Try regenerating your key from aistudio.google.com.`, { cause: err });
    }
    if (msg.includes('429')) {
      throw new Error('⚠️ Quota exceeded — Gemini rate limit hit. Wait a minute and retry.', { cause: err });
    }
    throw new Error(`Gemini error: ${msg}`, { cause: err });
  }
}
